...
- **Level A：机器记录 + 视觉证据。** 包含脱敏后的原生 usage/event 日志、运行前后截图和文件哈希。
- **Level B：视觉证据。** 包含足以连接配置、输入和结果的运行前后截图或连续录屏。
...
```text
README.md          项目入口和基本说明
CONTRIBUTING.md    中文贡献协议、证据规则和试运行 manifest
```

随着样板完成，预计逐步增加：

```text
runs/              每次测试的 manifest、输入、回复和脱敏证据
schemas/           正式数据 schema
prompts/           版本化的标准输入 case
scripts/           校验、脱敏检查、索引生成等辅助工具
docs/              方法说明、字段解释和中英文扩展文档
```

目录规划会根据前几次真实投稿调整，不会为了看起来完整而预先制造大量空目录。
