...
- 原图留在仓库外；
- 公开原图 SHA-256、脱敏副本 SHA-256 与固定遮挡坐标；
- 只用不透明纯色矩形遮挡，不裁切、不缩放、不模糊、不生成式重绘；
- 验证遮挡区域以外的 RGB 像素与全部 Alpha 通道像素均未改变；
- 发布最小脱敏转录和 transcript 事件；
- 发布经逐张目视检查与 OCR 隐私扫描的脱敏截图，视觉状态标为 `public`。

后续适合把这套操作封装成确定性脱敏辅助工具：只做不透明遮挡、字段转录、哈希和像素差异验证，不生成或重绘证据内容。工具必须保留原图哈希、公开遮挡坐标，并让维护者逐张目视验收。
