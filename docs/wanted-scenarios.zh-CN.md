...
5. 认领后按[实测指南](contributor-walkthrough.zh-CN.md)逐步执行；规则细节以[贡献指南](../CONTRIBUTING.md)为准。
...
Agent 的发行方和推理路由是两个变量：官方 Agent 也可以配置成走第三方网关（route 记 `third-party-gateway`）。中转站是社区里额度传闻最多、公开证据最少的一环——同名模型在中转站上的 token 计量、缓存行为和真实上游，目前几乎没有可复核的观察。规则见[贡献指南·官方产品、API 和中转站](../CONTRIBUTING.md#官方产品api-和中转站)。
